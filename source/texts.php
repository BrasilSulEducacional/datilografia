...
if (!empty($_POST)) {
    ...
    if (!empty($_POST["ranking"])) { //se finalizou o teste
        ...
        $studentBanned = $mysqli->query("SELECT * FROM ranking_banimentos WHERE codigo_aluno = '{$codigo}'");

        if ($studentBanned->num_rows) {
            echo json_encode(array_merge(["message" => "codigo banido"], $_POST));
            return;
        }
        ...
        if ($validStudent->num_rows) {
            ...
            if ($lastRank->num_rows) {
                ...
                if ($row["ppm"] < $ranking["ppm"]) {
                    ...
                    echo json_encode(array_merge(["message" => "atualizou sua pontuação"], $_POST));
                    return;
                }
            ...
            echo json_encode(array_merge(["mensagem" => "Opa, você está no ranking agora!"], $_POST));
    ...
    $query = $mysqli->query("SELECT * FROM test_texts ORDER BY RAND() LIMIT 1");
    $row = $query->fetch_assoc();

    $text = explode(" ", $row["text"]);
    $randText = array_rand($text, $maxWords);
    $words;

    foreach ($randText as $word) {
        $words[] = $text[$word];
    }
